...
<p>Viele Grüße,<br/>dein MTC Team</p>`,
  },
  {
    ...
    subject:
      "Neue Anmeldung zum Probetraining: {{local.event}} am {{strapi.date}}",
    html: `<h1>Neue Anmeldung zum Probetraining</h1>
<p><strong>Name:</strong> {{local.userName}}</p>
<p><strong>E-Mail:</strong> {{local.email}}</p>
<p><strong>Training:</strong> {{local.event}}</p>
<p><strong>Adresse:</strong> {{strapi.address}}</p>
<p><strong>Datum:</strong> {{strapi.date}}</p>`,
  },
];
